...
public class CrudExamples {
    ...
    public static String createEphemeralSequential(CuratorFramework client, String path,
        ...
        /*
            ...
            the caller.
         */
        return client.create().withProtection().withMode(CreateMode.EPHEMERAL_SEQUENTIAL)
    ...
    public static void setDataAsync(CuratorFramework client, String path, byte[] payload)
        ...
        CuratorListener listener = new CuratorListener() {
            ...
            public void eventReceived(CuratorFramework client, CuratorEvent event)
                ...
                if (Objects.equals(event.getType(), CuratorEventType.SYNC)) {
                    Logger.getGlobal().info(
                        "name:" + event.getName() + ",path:" + event.getPath() + ",data:"
...
//        client.getCuratorListenable().addListener(listener);
    ...
    public static void main(String[] args) throws Exception {
        ...
        client.start();
        client.getCuratorListenable().addListener((client1, event) -> Logger.getGlobal().info(
            "type:" + event.getType() + ",name:" + event.getName() + ",path:" + event.getPath()
                + ",data:" + Optional.ofNullable(event.getData()).map(String::new).orElse(null)));
        CrudExamples.watchedGetChildren(client, "/",
            watchedEvent -> Logger.getGlobal().info(watchedEvent.toString()));
        CrudExamples.create(client, "/test1/test1", "default".getBytes());
        CrudExamples.create(client, "/test2/test2", "default".getBytes());
        CrudExamples.setData(client, "/test1", "test1".getBytes());
        CrudExamples.setDataAsync(client, "/test2", "test2".getBytes());
        CrudExamples.guaranteedDelete(client, "/test1");
        CrudExamples.guaranteedDelete(client, "/test2");
